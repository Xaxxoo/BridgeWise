import { downloadCSV, downloadJSON } from '../utils/export-utils';
...
export async function exportTransactions(
  ...
  const response = await fetch(
    `${API_BASE_URL}/transactions/export/${format}?${queryParams.toString()}`,
    {
      method: 'GET',
      headers: {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorData.message || `Failed to export transactions: ${response.statusText}`
    );
  }
 ...
 */
export async function previewExportTransactions(
  format: 'csv' | 'json',
  filters: ExportFilters = {}
): Promise<string> {
  const queryParams = new URLSearchParams();
  ...
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}));
    throw new Error(
      errorData.message || `Failed to preview transactions: ${response.statusText}`
    );
  }
